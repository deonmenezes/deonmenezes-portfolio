...
function usageRows(keyRequests = 1, { globalRequests = 1, globalCost = 0 } = {}) {
  return [
    { key_hash: hashKey(VALID_KEY), requests: keyRequests, cost_usd: 0 },
    { key_hash: "*", requests: globalRequests, cost_usd: globalCost },
  ];
}
...
test("key creation returns a djev key and stores only its hash", async () => {
  ...
  const [ipHash, keyHash, prefix, perIp] = params;
...
test("evaluation proxies to the gateway and hides gateway metadata", async () => {
  ...
  const handler = createJevHandler({
    queryFn: async (sql, values) => { queries.push({ sql, values }); return usageRows(5); },
  ...
  assert.deepEqual(res.body, {
    ...
    confidence: {},
    usage: { inputTokens: 279, outputTokens: 20 },
    remainingToday: JEV_LIMITS.keyDailyRequests - 5,
  });
  ...
  assert.deepEqual(queries[0].values, [hashKey(VALID_KEY), "*"]);
  assert.deepEqual(queries[1].values, [hashKey(VALID_KEY), "*", 279, 0]);
});
...
test("evaluation enforces the per-key, site-wide, and billed-cost caps", async () => {
  const fetchFn = async () => { throw new Error("gateway should not be called"); };
  const cases = [
    [usageRows(JEV_LIMITS.keyDailyRequests + 1), "daily_limit_reached"],
    [usageRows(1, { globalRequests: JEV_LIMITS.globalDailyRequests + 1 }), "site_limit_reached"],
    [usageRows(1, { globalCost: JEV_LIMITS.globalDailyCostUsd }), "site_limit_reached"],
  ];
  for (const [rows, error] of cases) {
    const handler = createJevHandler({ queryFn: async () => rows, fetchFn });
    const res = response();
...
test("evaluation records what the gateway reports it billed", async () => {
  const queries = [];
  const handler = createJevHandler({
    queryFn: async (sql, values) => { queries.push(values); return usageRows(); },
    fetchFn: async () => gatewayOk("0.000011718"),
  });

  await handler(request(evaluation, { headers: { authorization: `Bearer ${VALID_KEY}` } }), response());

  assert.deepEqual(queries[1], [hashKey(VALID_KEY), "*", 279, 0.000011718]);
});
...
test("evaluation rejects oversized bodies", async () => {
  const handler = createJevHandler({ queryFn: async () => usageRows(), fetchFn: async () => gatewayOk() });
  const res = response();
