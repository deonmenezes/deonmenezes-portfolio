...
// Room for three long captions. Thinking is switched off in the request: left on, the
// model spent its whole allowance reasoning about a short caption and answered nothing.
  ...
  return async function handler(req, res) {
    ...
    try {
      upstream = await fetchFn(CHAT_URL, {
        ...
        body: JSON.stringify({ model: IDEAS_MODEL, max_tokens: MAX_OUTPUT_TOKENS, temperature: 0.8, thinking: { type: "disabled" }, messages }),
      });
