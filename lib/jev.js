...
// `requireKey: false` is for first-party pages (the viral simulator) that meter
// an anonymous bucket, such as a hashed network address, instead of a djev_ key.
const claimRequestSql = (requireKey) => `
  WITH active AS (
    ${requireKey
      ? "SELECT key_hash FROM jev_api_keys WHERE key_hash = $1 AND revoked_at IS NULL"
      : "SELECT $1::text AS key_hash"}
  ),
  current AS (
    SELECT
...
// Returns undefined for an unknown key, else { status, key_requests, day }.
// `bucket` lets a different paid service keep its own site-wide totals and caps, so
// spending there can never pause Jev, and Jev's budget can never be spent on it.
export async function claimRequest(queryFn, { usageKey, reservedTokens, dailyRequests, requireKey = true, bucket = {} }) {
  const limits = { ...JEV_LIMITS, ...bucket.limits };
  const [claim] = await queryFn(claimRequestSql(requireKey), [
    usageKey, bucket.globalKey || GLOBAL_USAGE_KEY, reservedTokens,
    dailyRequests, limits.globalDailyRequests,
    limits.globalDailyInputTokens, limits.globalDailyCostUsd,
  ]);
  return claim;
}

export async function reconcileUsage(queryFn, { usageKey, reservedTokens, claim, payload, bucket = {} }) {
  const inputTokens = Number(payload?.usage?.inputTokens) || 0;
  const cost = Number(payload?.providerMetadata?.gateway?.cost) || 0;
  try {
    await queryFn(RECONCILE_USAGE_SQL, [usageKey, bucket.globalKey || GLOBAL_USAGE_KEY, inputTokens, reservedTokens, cost, claim.day]);
  } catch (error) {
...
// One short retry on 429: the gateway asks callers to retry rather than fail.
export async function callJev({ state, questions, gatewayKey, fetchFn, retryDelayMs = 1200 }) {
  for (let attempt = 0; ; attempt++) {
    const upstream = await fetchFn(GATEWAY_URL, {
      method: "POST",
...
export function createJevHandler({ queryFn = query, fetchFn = (...args) => fetch(...args), retryDelayMs } = {}) {
  return async function handler(req, res) {
    applyCors(res);
    ...
    let claim;
    try {
      claim = await claimRequest(queryFn, { usageKey: keyHash, reservedTokens, dailyRequests: JEV_LIMITS.keyDailyRequests });
    } catch (error) {
    ...
    let payload;
    try {
      ({ upstream, payload } = await callJev({ state: body.state, questions: body.questions, gatewayKey, fetchFn, retryDelayMs }));
    } catch (error) {
    ...
    const inputTokens = await reconcileUsage(queryFn, { usageKey: keyHash, reservedTokens, claim, payload });
