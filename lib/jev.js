...
export async function claimRequest(queryFn, { usageKey, reservedTokens, dailyRequests, requireKey = true }) {
  const [claim] = await queryFn(claimRequestSql(requireKey), [
    usageKey, GLOBAL_USAGE_KEY, reservedTokens,
    dailyRequests, JEV_LIMITS.globalDailyRequests,
    JEV_LIMITS.globalDailyInputTokens, JEV_LIMITS.globalDailyCostUsd,
  ]);
  return claim;
}

export async function reconcileUsage(queryFn, { usageKey, reservedTokens, claim, payload }) {
  const inputTokens = Number(payload?.usage?.inputTokens) || 0;
  const cost = Number(payload?.providerMetadata?.gateway?.cost) || 0;
  try {
    await queryFn(RECONCILE_USAGE_SQL, [usageKey, GLOBAL_USAGE_KEY, inputTokens, reservedTokens, cost, claim.day]);
  } catch (error) {
