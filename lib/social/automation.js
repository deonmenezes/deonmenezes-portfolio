...
export function normalizeFlowSteps(value) {
  ...
  }).filter((step) => step.type === "button" ? step.text && step.buttons.length : step.text);
}
...
export function buildFlowMessages(automation, deliveryId, baseUrl) {
  ...
  if (!steps.length) return [{ text: buildDeliveryText(automation, deliveryId, baseUrl) }];
  const messages = steps.map((step, index) => {
    if (step.type === "button") {
      const buttons = step.buttons.map((button) => button.type === "postback"
        ...
        : { type: "web_url", title: button.title, url: new URL(button.url).toString() });
...
async function sendFlowMessages({ messages, commentId, recipientId, username, deliveryId }) {
  let firstResult = null;
  for (const [index, message] of messages.entries()) {
    const result = index === 0 && commentId
      ? await privateReply(commentId, message)
      : await sendMessage(recipientId, message);
    if (!firstResult) firstResult = result;
    const messageId = String(result?.message_id || `delivery:${deliveryId}:step:${index}`);
    await query(
      ...
      [messageId, recipientId, username, messageBody(message)],
    );
  }
  return firstResult || {};
}
...
async function processComment(event, baseUrl) {
  ...
  try {
    ...
    const flowMessages = gated ? [buildFollowCard(delivery.id)] : buildFlowMessages(automation, delivery.id, baseUrl);
...
async function processPostback(event, baseUrl) {
  ...
  try {
    const result = await sendFlowMessages({
      messages: buildFlowMessages({ ...delivery, id: delivery.automation_id }, delivery.id, baseUrl),
...
async function retryPendingDeliveries(baseUrl, limit = 20) {
  const rows = await query(
    `SELECT d.*,a.response_text,a.public_reply_text,a.resource_links,a.flow_steps,a.follow_gate_mode,a.id AS automation_id
  ...
  for (const delivery of rows) {
    ...
    try {
      ...
      const result = await sendFlowMessages({
        messages: gated ? [buildFollowCard(delivery.id)] : buildFlowMessages({ ...delivery, id: delivery.automation_id }, delivery.id, baseUrl),
        commentId: resourceAction ? null : delivery.comment_id,
        recipientId,
        username: delivery.commenter_username,
        deliveryId: delivery.id,
      });
