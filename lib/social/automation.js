...
async function sendFlowMessages({ plan, messages, commentId, recipientId, username, deliveryId, automationId }) {
  ...
  for (const [index, entry] of entries.slice(1).entries()) {
    ...
    await query(
      `INSERT INTO social_flow_queue
       (delivery_id,automation_id,recipient_id,step_index,message,available_at)
       VALUES ($1,$2,$3,$4,$5::jsonb,now()+($6 * interval '1 second'))
       ON CONFLICT (delivery_id,step_index) DO NOTHING`,
      [deliveryId, automationId, recipientId, stepIndex, JSON.stringify(entry.message), scheduleOffset],
    );
  }
  return firstResult || {};
}
...
async function processComment(event, baseUrl) {
  ...
  try {
    ...
    const flowPlan = gated ? [{ delaySeconds: 0, message: buildFollowCard(delivery.id) }] : buildFlowPlan(automation, delivery.id, baseUrl, { follows, text: payload.text, tags });
...
async function processPostback(event, baseUrl) {
  ...
  if (continueMatch && payload.senderId) {
    ...
    await query(
      `UPDATE social_flow_queue SET available_at=now()
...
async function processFlowQueue(limit = 20) {
  const rows = await query(
    `SELECT q.id,q.delivery_id,q.recipient_id,q.step_index,q.message,q.attempts
     ...
     WHERE q.status IN ('pending','retryable') AND q.available_at <= now()
       AND NOT EXISTS (
