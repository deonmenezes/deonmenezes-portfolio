...
// Meta can return a transient 5xx (or a rate limit) after the request has
// passed its own transport retries. Keep those deliveries in the durable
// queue so the worker can try again instead of leaving them permanently
// failed. Transport failures are normalized to status 503 by meta.js;
// four hundred-level errors remain terminal because retrying them cannot
// repair an invalid permission, recipient, or payload.
export function isRetryableError(error, attempts = 1, maxAttempts = 5) {
  if (Number(attempts) >= maxAttempts) return false;
      ...
      if (keyword) {
        await query(
          `INSERT INTO social_automations
           (media_id, trigger_type, trigger_key, title, keyword, enabled, needs_setup, source)
           VALUES ($1,'comment',$4,$2,$3,false,true,'caption_discovery')
           ON CONFLICT (media_id) DO NOTHING`,
          [item.id, titleFromCaption(item.caption, mediaShortcode), keyword, `comment:${item.id}`],
        );
      }
    }
...
async function processComment(event, baseUrl) {
  ...
  try {
    ...
    const retryable = isRetryableError(error, Number(delivery.attempts || 1));
    await query(
...
async function processFlowQueue(limit = 20) {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      const retryable = isRetryableError(error, attempts);
      await query(
...
async function retryPendingDeliveries(baseUrl, limit = 20) {
  const rows = await query(
    `SELECT d.*,a.response_text,a.public_reply_text,a.resource_links,a.flow_steps,a.follow_gate_mode,a.trigger_type,a.id AS automation_id
     ...
     WHERE d.status='retryable' AND COALESCE(d.next_attempt_at, now()) <= now()
...
export async function processPendingEvents(baseUrl, limit = 20) {
  ...
  const rows = await query(
    `SELECT id, event_type, payload FROM social_webhook_events
     WHERE status IN ('pending','retryable') AND COALESCE(next_attempt_at, now()) <= now()
     ORDER BY id LIMIT $1`,
    [Math.max(1, Math.min(100, Number(limit) || 20))],
  );
  ...
  for (const row of rows) {
    ...
    try {
      ...
      const retry = isRetryableError(error, attempts);
