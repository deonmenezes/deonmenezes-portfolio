...
export function parseGoogleTrends(xml, place) {
  ...
  for (const [, item] of String(xml).matchAll(/<item>([\s\S]*?)<\/item>/gu)) {
    ...
    if (text) topics.push(`${text} (${place})`.slice(0, MAX_TOPIC_CHARS + 8));
...
const apify = (actor, input, pick, maxChargeUsd) => async (fetchFn) => {
  ...
  if (!response.ok) throw new Error(`apify ${actor} ${response.status}`);
...
export const SOURCES = {
  ...
  tiktok: {
    ...
    everyMs: 48 * HOUR,
    platforms: ["tiktok"],
    run: apify("clockworks~tiktok-trends-scraper", { adsScrapeHashtags: true, resultsPerPage: MAX_PER_SOURCE, adsCountryCode: "US", adsTimeRange: "7" }, (item) => (item?.name ? `#${String(item.name).replace(/^#/u, "")}` : ""), 0.06),
  },
  instagram: {
...
export async function refreshDue(db, { fetchFn = (...args) => fetch(...args), now = Date.now(), sources = SOURCES } = {}) {
  ...
  await Promise.all(Object.entries(sources).map(async ([id, source]) => {
    ...
    try {
      ...
      await collection.updateOne({ _id: id }, { $set: { error: String(error?.message || error).slice(0, 120), triedAt: new Date(now) } }).catch(() => {});
    }
  }));
}
