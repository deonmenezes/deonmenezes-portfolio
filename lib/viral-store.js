...
const FEED_FIELDS = { projection: { "comments.text": 0, "comments.author": 0 } };
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      const [recent, top] = await Promise.all([
        collection.find({ platform }, FEED_FIELDS).sort({ createdAt: -1 }).limit(FEED_LIMIT).toArray(),
        collection.find({ platform, createdAt: { $gte: since } }, FEED_FIELDS).sort({ viralScore: -1, createdAt: -1 }).limit(LEADERBOARD_LIMIT).toArray(),
      ]);
