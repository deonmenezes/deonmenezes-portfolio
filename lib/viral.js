...
import { authRequired, currentUser, noteSimulation } from "./viral-auth.js";
...
export function createViralHandler({ queryFn = query, fetchFn = (...args) => fetch(...args), retryDelayMs, saveFn = savePost, storeEnabledFn = storeEnabled, creatorFn = cachedCreator, liveTrendsFn = liveTrendsFor, authRequiredFn = authRequired, userFn = currentUser, noteFn = noteSimulation } = {}) {
  return async function handler(req, res) {
    ...
    // Posting needs an account once sign-in is set up; reading the feed never does.
    let user = null;
    if (authRequiredFn()) {
      user = await userFn(req).catch((error) => {
        logError("viral.auth", error);
        return null;
      });
      if (!user) return sendJson(res, 401, { error: "sign_in_required", message: "Sign in to post." });
    }
    ...
    if (body?.publish === true && author && typeof body?.id === "string" && storeEnabledFn()
      ...
      try {
        published = await saveFn({ id: body.id, platform: platformId, text, extra, format, attachments, poll, author, scored, userId: user?._id });
      } catch (error) {
        logError("viral.save", error);
      }
    }

    if (user) await noteFn(user, platformId);
