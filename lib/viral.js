...
export function describePost({ platformId, text, extra, format, attachments, poll, visual = "", topics = [], creator, now = Date.now() }) {
  const media = {
  ...
  return {
    ...
    ...trendContext(platformId, now, topics),
  };
}
...
export function createViralHandler({ queryFn = query, fetchFn = (...args) => fetch(...args), retryDelayMs, saveFn = savePost, storeEnabledFn = storeEnabled, creatorFn = cachedCreator } = {}) {
  return async function handler(req, res) {
    ...
    const state = describePost({ platformId, text, extra, format, attachments, poll, visual, topics, creator });
    ...
    const scored = scorePost(payload.answers, { platform: platformId, followers: known?.followers ?? followers, textLength: text.length });
