...
import { trendContext } from "../viral-trends.js";
...
export function describePost({ platformId, text, extra, format, attachments, poll, visual = "", now = Date.now() }) {
  const media = {
...
export function createViralHandler({ queryFn = query, fetchFn = (...args) => fetch(...args), retryDelayMs, saveFn = savePost, storeEnabledFn = storeEnabled } = {}) {
  return async function handler(req, res) {
    ...
    const state = describePost({ platformId, text, extra, format, attachments, poll, visual });
